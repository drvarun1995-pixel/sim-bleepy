-- PERMANENT FIX FOR EVENTS AUTHOR SYSTEM - PRESERVE ORIGINAL AUTHOR NAMES
-- This version preserves the original author_name from the database
...
-- =====================================================
-- 2. FIX EXISTING DATA ISSUES - PRESERVE AUTHOR NAMES
-- =====================================================
...
-- Fix events with missing author_id but keep original author_name
-- For the "test" event with "VT NHS", we'll link it to the Dr. Varun user but keep "VT NHS" as the author_name
...
WHERE (author_id IS NULL OR author_name = 'Unknown User')
  AND EXISTS (
      SELECT 1 FROM public.users 
      WHERE (name = 'Dr. Varun' OR email LIKE '%drvarun19%')
  )
  -- Keep the original author_name, don't change it
  AND author_name IS NOT NULL 
  AND author_name != 'Unknown User';

-- For events that have no author_name at all, set a default
UPDATE public.events 
SET 
    author_id = (
        SELECT id FROM public.users 
...
WHERE author_id IS NULL 
  AND (author_name IS NULL OR author_name = 'Unknown User')
  AND EXISTS (
      SELECT 1 FROM public.users 
      WHERE (name = 'Dr. Varun' OR email LIKE '%drvarun19%')
  );
...
-- =====================================================
-- 4. CREATE COMPREHENSIVE EVENTS VIEW - PRESERVE AUTHOR NAMES
-- =====================================================
...
SELECT 
  ...
  e.more_info_target,
  e.event_status,
  -- PRESERVE ORIGINAL AUTHOR NAME - this is the key change
  e.author_id,
  e.author_name,  -- Use the original author_name from the database
  u.email AS author_email,
  u.role AS author_role,
  u.display_name AS author_display_name,
  u.name AS author_user_name,  -- Show the actual user name for reference
  e.created_by,
...
BEGIN
  ...
  IF current_user_id IS NOT NULL THEN
    ...
    -- Only set author_name if it's not already provided
...
-- Show sample of fixed events with preserved author names
SELECT 
    id,
    title,
    author_id,
...
SELECT 'PERMANENT FIX COMPLETE! Original author names preserved, events now properly linked to users.' as status;
