...
async function getUserRole(userEmail: string): Promise<'admin' | 'educator' | 'student' | 'meded_team' | 'ctf'> {
  try {
    ...
    console.log('My certificates layout - fetching role for:', userEmail, { role: user?.role, error: error?.message })

    if (error || !user) {
      console.log('My certificates layout - defaulting to student role')
      return 'student'
    }

    const userRole = user.role || 'student'
    console.log('My certificates layout - resolved role:', userRole)
    return userRole as 'admin' | 'educator' | 'student' | 'meded_team' | 'ctf'
  } catch (error) {
    console.error('My certificates layout - error fetching role:', error)
    return 'student'
  }
}

export default async function MyCertificatesLayout({
  children,
}: {
