...
const SIGNATURE_BUCKET_ID = 'email-files'
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('Signature image compression failed, using original buffer:', error)
      processedBuffer = originalBuffer
    }

    const fileName = `${Date.now()}-${Math.random().toString(36).slice(2)}.webp`
    const folderPath = `${SIGNATURE_PREFIX}/${user.id}/images`
    const filePath = `${folderPath}/${fileName}`

    const { error: uploadError } = await supabaseAdmin.storage
      .from(SIGNATURE_BUCKET_ID)
    ...
    if (uploadError) {
      console.error('Failed to upload signature image:', uploadError)
    ...
    console.error('Error in POST /api/emails/signatures/images:', error)
