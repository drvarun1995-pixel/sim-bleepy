...
import { calendarCohortLabel, compareCohortLabels, previousCohortLabel, workingCohortLabel } from '@/lib/year-progression'
...
function EmailNewsletterPage() {
  ...
  const visibleRecipients = useMemo(() => {
    ...
    return [...matchingUsers, ...extras, ...searchHits].filter((user) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const nextUsers = (json.users || []).filter(
          (user: NewsletterUser) =>
            canReceiveNewsletter(user) && !isExcludedFromLearnerLists(user)
        )
        setUsers(nextUsers)
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Input
                  ...
                  placeholder="Search, or type a name to add someone else"
                />
                <div className="max-h-96 divide-y overflow-y-auto rounded-lg border">
                  {visibleRecipients.length === 0 ? (
                    <p className="p-4 text-sm text-slate-500">No people in this category for the selected cohort.</p>
                  ) : (
