-- Supabase SQL Script to Check Feedback Forms
-- Run this in your Supabase SQL Editor
--
-- NOTE: If you get errors about missing columns (like anonymous_enabled), 
-- you may need to run the migration script first:
-- migrations/add-anonymous-feedback.sql
--
-- 0. Check if anonymous_enabled column exists (run this first)
SELECT column_name, data_type, is_nullable
FROM information_schema.columns 
WHERE table_name = 'feedback_forms' 
AND column_name = 'anonymous_enabled';

-- 1. Check all feedback forms with basic info
SELECT 
    id,
    form_name,
    form_template,
    active,
    COALESCE(anonymous_enabled, false) as anonymous_enabled,
    created_at,
    event_id,
    created_by
FROM feedback_forms
...
SELECT 
    ...
    ff.form_template,
    ff.active,
    COALESCE(ff.anonymous_enabled, false) as anonymous_enabled,
    ff.created_at,
