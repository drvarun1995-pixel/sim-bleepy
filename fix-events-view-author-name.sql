-- Fix the events_with_details view to prioritize author_name over user name
...
-- Check what the current view returns
SELECT 
    id,
    title,
    author_id,
...
-- Drop and recreate the view to prioritize author_name
...
SELECT 
  ...
  e.more_info_target,
  e.event_status,
  -- CRITICAL: Preserve the original author_name from events table
  e.author_id,
  e.author_name,  -- Use the original author_name from events table
  u.email AS author_email,
  u.role AS author_role,
  u.display_name AS author_display_name,
  u.name AS author_user_name,  -- Show the linked user's name for reference
  e.created_by,
...
SELECT 'Events view updated to preserve original author names!' as status;
