-- =====================================================
-- FIX RLS POLICIES FOR AUTHENTICATED USER OPERATIONS (CORRECTED)
-- =====================================================
...
--
-- CORRECTED: Removed policies for non-existent tables (event_categories, event_formats)
-- =====================================================
...
-- Event Speakers Junction (ONLY IF IT EXISTS)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'event_speakers') THEN
    EXECUTE 'CREATE POLICY "Authenticated users can insert event_speakers"
    ...
    EXECUTE 'CREATE POLICY "Authenticated users can delete event_speakers"
...
-- Event Locations Junction (ONLY IF IT EXISTS)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'event_locations') THEN
    EXECUTE 'CREATE POLICY "Authenticated users can insert event_locations"
    ...
    EXECUTE 'CREATE POLICY "Authenticated users can delete event_locations"
...
-- Event Organizers Junction (ONLY IF IT EXISTS)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'event_organizers') THEN
    EXECUTE 'CREATE POLICY "Authenticated users can insert event_organizers"
    ...
    EXECUTE 'CREATE POLICY "Authenticated users can delete event_organizers"
...
--
-- CORRECTED VERSION:
-- - Removed policies for event_categories (doesn't exist)
-- - Removed policies for event_formats (doesn't exist)
-- - Added conditional checks for junction tables
-- - Added conditional check for resources table
--
